...
function setFeatureValue(state: FeaturesState, name: string, value: boolean | undefined): FeaturesState {
...
describe('testFeature', () => {
  describe('single state machine', () => {
    ...
    it('should return true when feature is enabled', () => {
      let state = createFeaturesState([{ name: 'Feature1', defaultValue: false }]);
    ...
    it('should return false when feature is disabled', () => {
      let state = createFeaturesState([{ name: 'Feature1', defaultValue: true }]);
    ...
    it('should return default value when no override is set', () => {
      const state = createFeaturesState([{ name: 'Feature1', defaultValue: true }]);
      const result = testFeature('Feature1', [state]);
      expect(result).toBe(true);
    });
  });
  ...
  describe('forced values', () => {
    it('should prioritize forced value over non-forced value', () => {
      ...
      service1.send({
        type: 'INIT',
        features: [{ name: 'Feature1', description: 'Test', defaultValue: false, force: true }],
      });
      let state1 = service1.getSnapshot();
      state1 = setFeatureValue(state1, 'Feature1', true);

      // Create state with force=false and value=false
      let state2 = createFeaturesState([{ name: 'Feature1', defaultValue: false, force: false }]);
      state2 = setFeatureValue(state2, 'Feature1', false);
